...
function photoClip(container, option) {
			...
			fileReader.onload = function(e) {
				...
				if (kbs > 1024) {
					...
					$tempImg.load(function() {
						...
						createImg(newDataURL);console.log(1)
					});
					$tempImg.attr("src", this.result);
					// console.log(this.result);
				} else {
					createImg(this.result);console.log(2)
					// console.log(this.result);
				}
			};
	...
	var picnum = parseInt(Math.random()*3) + 1;
	function clipImg() {
		if (!imgLoaded) {
			// alert("亲，当前没有图片可以裁剪!");
			$(".card").attr("src","img/card"+picnum+".jpg")
	...
	function init() {
		...
		var $mask = $("<div class='photo-clip-mask'>").css({
			...
			"height": "100%",
			"pointer-events": "none",
			// "background-size": clipWidth,
			"background-position-x": (ww-clipWidth)/2,
			"background-position-y": 0.5*hh-clipHeight/2,
			"background": "url(../fatherday/img/bg.png)",
			"background-size": "100%", 
		}).appendTo($container);
		// var $mask1 = $("<div class='photo-clip-mask1'>").css({
		// 	"position": "absolute",
		// 	"left": "50%",
		// 	"top": "45%",
		// 	"width": clipWidth,
